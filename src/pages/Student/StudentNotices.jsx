      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(4)].map((_, index) => (
            <div
              key={index}
              className="h-28 animate-pulse rounded-3xl border border-slate-200 bg-white shadow-soft"
            />
          ))}
        </div>
      ) : error ? (
          ...
          {notices.map((notice) => {
            ...
            return (
              <CardElement
                ...
                {...extraProps}
                className={[
                  'block group rounded-3xl border border-slate-200 bg-white p-5 shadow-soft transition-all duration-300 hover:border-brand/20 hover:shadow-md',
                  notice.noticeLink ? 'cursor-pointer' : '',
                ].join(' ')}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-2 min-w-0">
                    <h3 className="text-lg font-bold text-slate-900 wrap-break-word group-hover:text-brand transition-colors">
                      {notice.title}
                    </h3>
                  ...
                  {notice.noticeLink ? (
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-50 text-slate-500 group-hover:bg-blue-50 group-hover:text-brand transition">
